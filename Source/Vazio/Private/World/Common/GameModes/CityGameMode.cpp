...
ACityGameMode::ACityGameMode()
{
	...
	// Tentar carregar o BP do personagem e us�-lo como DefaultPawnClass
	...
	// Carregar assets no constructor (onde FObjectFinder � permitido)
...
{
	// Criar o resto do ambiente ap�s PlayerStart estar pronto
	CreateCityGround();
	CreateBasicLighting();
...
void ACityGameMode::CreatePlayerStartIfNeeded()
{
	...
	// Verificar se j� existe um PlayerStart
	for (TActorIterator<APlayerStart> ActorItr(World); ActorItr; ++ActorItr)
	{
		UE_LOG(LogTemp, Warning, TEXT("[CityGameMode] PlayerStart j� existe"));
		return;
	}

	// Criar PlayerStart no centro da cidade com rota��o e configura��es adequadas
...
void ACityGameMode::CreateCityGround()
{
	...
	if (!World) 
	{
		UE_LOG(LogTemp, Error, TEXT("[CityGameMode] World � NULL"));
		return;
	}

	if (!CubeMesh)
	{
		UE_LOG(LogTemp, Error, TEXT("[CityGameMode] CubeMesh n�o foi carregado!"));
		return;
	}
	...
	if (GroundActor)
	{
		...
		// Aplicar cor verde para o ch�o ser vis�vel
		if (BasicMaterial)
		{
			// Criar uma inst�ncia din�mica do material para mudar a cor
			UMaterialInstanceDynamic* DynamicMaterial = UMaterialInstanceDynamic::Create(BasicMaterial, GroundActor);
			if (DynamicMaterial)
			{
				// Definir cor verde para o ch�o
...
#endif
		UE_LOG(LogTemp, Warning, TEXT("[CityGameMode] Ch�o VERDE da cidade criado: 30x30m - Posi��o: %s"), *GroundLocation.ToString());
	}
	else
	{
	...
	// Criar alguns cubos de refer�ncia coloridos
...
void ACityGameMode::CreateReferenceCubes()
{
	...
	// Criar 4 cubos coloridos para refer�ncia visual
...
#endif
			UE_LOG(LogTemp, Warning, TEXT("[CityGameMode] Cubo %d criado na posi��o: %s"), i, *CubePositions[i].ToString());
		}
	}
	
	UE_LOG(LogTemp, Warning, TEXT("[CityGameMode] 4 cubos de refer�ncia coloridos criados com Mobility corrigida"));
}
...
void ACityGameMode::CreateBasicLighting()
{
	...
	// Criar Sky Light para ilumina��o ambiente
...
AActor* ACityGameMode::ChoosePlayerStart_Implementation(AController* Player)
{
	...
	// Se n�o encontrou nenhum, for�ar cria��o de um tempor�rio
